...
export function saveBookToDB(bookObj) {
...
export function getAllBooksFromDB() { return new Promise((resolve) => { if (!db) return resolve([]); const req = db.transaction("library", "readonly").objectStore("library").getAll(); req.onsuccess = () => resolve(req.result || []); req.onerror = () => resolve([]); }); }
export function getBookFromDB(id) { return new Promise((resolve) => { if (!db || id === "schnellstart") return resolve(null); const req = db.transaction("library", "readonly").objectStore("library").get(id); req.onsuccess = () => resolve(req.result || null); req.onerror = () => resolve(null); }); }
...
export function saveAppState(key, val) { return new Promise((resolve) => { if (!db) { localStorage.setItem(key, JSON.stringify(val)); return resolve(); } const tx = db.transaction("appState", "readwrite"); tx.objectStore("appState").put(val, key); tx.oncomplete = () => resolve(); }); }
export function getAppState(key) { return new Promise((resolve) => { if (!db) { const val = localStorage.getItem(key); try { resolve(val ? JSON.parse(val) : null); } catch { resolve(val); } return; } const req = db.transaction("appState", "readonly").objectStore("appState").get(key); req.onsuccess = () => resolve(req.result); req.onerror = () => resolve(null); }); }
export function saveToStatsArchive(entry) { return new Promise((resolve) => { if (!db) return resolve(); const tx = db.transaction("statsArchive", "readwrite"); tx.objectStore("statsArchive").put(entry); tx.oncomplete = () => resolve(); }); }
export function deleteFromStatsArchive(id) {
    return new Promise((resolve) => {
        if (!db) return resolve();
...
export function getAllStatsArchive() { return new Promise((resolve) => { if (!db) return resolve([]); const req = db.transaction("statsArchive", "readonly").objectStore("statsArchive").getAll(); req.onsuccess = () => resolve(req.result || []); req.onerror = () => resolve([]); }); }

// ── libraryContent: schwere Daten (words, text, cover) ──────────────────────
export function saveBookContentToDB(id, words, text, cover) {
    return new Promise((resolve) => {
        if (!db) return resolve();
...
export function getBookContentFromDB(id) {
    return new Promise((resolve) => {
...
export function deleteBookContentFromDB(id) {
    return new Promise((resolve) => {
        if (!db) return resolve();
        const tx = db.transaction("libraryContent", "readwrite");
...
export async function migrateLibraryContent() {
    ...
    for (const book of books) {
        ...
        // estimatedTotalSeconds und thumbnail werden nach Migration in init() gesetzt
        delete meta.words; delete meta.text; delete meta.cover;
